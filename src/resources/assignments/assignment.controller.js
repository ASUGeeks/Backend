...
async function grade_submission(req, res) {
    const token = req.headers.token;
    if (!token) return res.status(401).send("Forbidden");

    const sub_id = req.body.submission_id;
    if (!sub_id) return res.status(404).send({ message: "Please send submission id" });
    ...
    try {
        const sub = await Submission.findByIdAndUpdate(sub_id, {
...
module.exports = { create_assignment, submit_assignment, get_assignments, get_submissions, grade_submission };